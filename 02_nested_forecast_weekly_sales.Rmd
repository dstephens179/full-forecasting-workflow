---
title: "Nested Sales Forecast - by Tienda, Owner, Metal Type & Product"
---
...
data_filtered_tbl <- data_tbl %>%
  ...
  # filter(metal_type == "10" | metal_type == "14" | metal_type == "Diamante") %>%
  
  # filter(str_detect(tienda, "Centro") |
...
data_filtered_tbl %>%
  summarize_by_time(.date_var = date, .by = "week", value = sum(sales)) %>%
...
weekly_corrected_tbl %>%
  ...
  group_by(tienda) %>%
  summarize_by_time(.date_var = date, .by = "month", sales = sum(sales)) %>%
...
# 3.0 RECIPES
```{r}
recipe_spec <- recipe(sales ~., extract_nested_train_split(nested_data_tbl)) %>%
...
recipe_spec_ml <- recipe(sales ~., extract_nested_train_split(nested_data_tbl)) %>%
  step_timeseries_signature(date) %>%
  step_rm(matches("(.xts$)|(.iso$)|(hour)|(minute)|(second)|(am.pm)")) %>%
  step_normalize(matches("(index.num)|(year)|(yday)")) %>%
  step_dummy(all_nominal(), one_hot = TRUE) %>%
...
best_forecast_models_tbl <- nested_best_refit_tbl %>%
  ...
  mutate(
    .model_desc = ifelse(.key == "actual", "actual", "forecast"),
    forecast = ifelse(.value<0, 0, round(.value, 0)),
    date = .index) %>%
...
best_forecast_models_tbl %>% write_rds("00_models/best_forecast_models_tbl.rds")

best_forecast_models_tbl <- read_rds("00_models/best_forecast_models_tbl.rds")
